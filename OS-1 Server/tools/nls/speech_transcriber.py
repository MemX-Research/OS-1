...
from tools.nls.core import NlsCore
from tools.nls.exception import (
...
class NlsSpeechTranscriber:
    ...
    def __init__(
        ...
        self.__allow_aformat = ("pcm", "opus", "opu")
    ...
    def stop(self, timeout=10):
        ...
        try:
            ...
            logging.error("exception on stop(): {}".format(e))
            self.__tr_core_on_close()
    ...
    def send_audio(self, pcm_data):
        ...
        with self.__start_cond:
            if not self.__start_flag:
                logging.debug("not start yet...")
                raise NotStartException("not start yet")
                # return
        try:
